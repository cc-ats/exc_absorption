    ...
    for json_index, json_file in enumerate(json_files):
        ...
        try:
            ...
            if 'interactions' in data and len(data['interactions']) >= 1:
                ...
                if distance_label not in interaction_terms:
...
def build_matrix(coordinates, identifiers, interaction_terms, r_cutoffs=[8, 10], z_cutoff=np.inf, coupling='J_dipole'):
    ...
    for mi1, coord1 in enumerate(coordinates):
        mi1_identifier = identifiers[mi1]

        # loop over pairwise interaction
        for mi2, coord2 in enumerate(coordinates[mi1+1:], start=mi1+1):
            mi2_identifier = identifiers[mi2]
            ...
            for ri, r_max in enumerate(r_cutoffs):
                ...
                if distance < r_max and distance >= r_min and distance_z <= z_cutoff: # remove if not used z_cutoff
                    ...
                    closest_match = 0
                    closest_match_val = 0
                    interaction_term = interaction_terms.get(distance_label, None)
                    for term_index, [term_distance_vec, term_results] in enumerate(interaction_term):
                        ...
                        if np.abs(term_match_val) > np.abs(closest_match_val):
                            closest_match = term_index
                            closest_match_val = term_match_val

                    transpose = False
                    if closest_match_val < 0:
                        if mi1_identifier == mi2_identifier == '2':
                            transpose = True
                    elif mi1_identifier == mi2_identifier == '1':
                        transpose = True

                    print(f'parameters: ({2*mi1:02}x{2*mi2:02}), distance:{distance_label}, {closest_match_val:.3f}, {closest_match}, {transpose}, identifiers:({mi1_identifier},{mi2_identifier}), interactions: {interaction_term[closest_match][1][coupling].flatten()[1]:.4f}, {interaction_term[closest_match][1][coupling].flatten()[2]:.4f}')
                    ...
                    matrix[2*mi2:2*mi2+2,2*mi1:2*mi1+2] = matrix[2*mi1:2*mi1+2,2*mi2:2*mi2+2].T


        # molecule energies and dipoles are same
        interaction_term = [val for key, val in interaction_terms.items()][0]

        # Fill in the diagonal terms (Qs)
        matrix[2*mi1+0, 2*mi1+0] = interaction_term[0][1]['Qx1']
        matrix[2*mi1+1, 2*mi1+1] = interaction_term[0][1]['Qy1']
